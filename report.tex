...
contredira partiellement l'intuition avancée en 4.4, selon laquelle le bénéfice du filtrage devrait être concentré dans les périodes de forte volatilité. Une explication plausible est que la classification par volatilité \emph{réalisée du marché} (S\&P~500) ne coïncide pas nécessairement avec la présence de sauts \emph{idiosyncrasiques sur le spread de la paire} : un saut sur le spread MA/V ou HD/LOW peut survenir indépendamment d'un pic de volatilité agrégée du marché (annonce spécifique à l'entreprise, rotation sectorielle, rebalancement d'indice), de sorte qu'une fraction substantielle des sauts filtrés en régime de basse volatilité de marché correspond à du bruit idiosyncratique que le $z$-score naïf interprète à tort comme un signal de retour à la moyenne exploitable. Ce résultat, obtenu sur un échantillon restreint, doit être vu comme une piste de recherche plutôt qu'une conclusion établie : il suggérerait qu'un régime construit sur la volatilité \emph{du spread lui-même} (ou sur $\pi_t$ agrégé entre les paires) serait un meilleur candidat de segmentation qu'un régime de marché agrégé. \textbf{Avertissement de reproductibilité} : contrairement au reste de cette sous-partie et de la sous-partie 4.7 (config. de référence, grille de sensibilité, sous-périodes, DSR), qui se reproduisent exactement à partir de \texttt{robustness\_4\_7.py} sur les données actuellement disponibles, ce tableau de segmentation par régime n'a pu être reproduit avec aucune des deux implémentations archivées (\texttt{backtest\_4\_6.py} ou une extension de \texttt{robustness\_4\_7.py} appliquant la même segmentation à sa propre config. de référence) : les deux donnent des couples (Sharpe vol. haute, Sharpe vol. basse) sensiblement différents de ceux ci-dessus, pour la stratégie naïve comme pour la filtrée. Le signe qualitatif du schéma décrit ci-dessus (surperformance de la filtrée dans les deux régimes, plus marquée en régime calme) n'a donc pas pu être revérifié de façon indépendante et doit être lu avec la même prudence que les autres résultats non stables signalés en 4.7.
